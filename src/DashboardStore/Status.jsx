...
function Status({ storeID, isLoggedIn }) {
    ...
    useEffect(() => {
        if (storeID) {
            ...
            fetch(`https://backendthirdv-n0dx.onrender.com/api/history-count2/${storeID}`)
    ...
    useEffect(() => {
        ...
        const fetchReportData = async () => {
            try {
                const response = await fetch(`https://backendthirdv-n0dx.onrender.com/api/report-count-per-month/${storeID}`);
