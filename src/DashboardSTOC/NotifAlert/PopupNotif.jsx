    ...
    useEffect(() => {
        const checkAndOpenPopup = () => {
            if (latestReports.length > 0) {
                ...
                const dismissedId = parseInt(sessionStorage.getItem("dismissedReportId"), 10);

                console.log("🆕 New Report ID:", latestReportId);
                console.log("🚫 Dismissed Report ID from session:", dismissedId);

                if (latestReportId !== dismissedId) {
                    openPopupWindow(latestReports[0], latestReportId);
                    setLastReportId(latestReportId);
                } else {
                    console.log("🚫 PopUp already dismissed, not showing again.");
                }
            }
        };
